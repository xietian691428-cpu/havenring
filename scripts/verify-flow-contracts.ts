...
check("pair model: haven-scoped sync and owner-only seal", () => {
  ...
  assert.match(readRepoFile("public/sw.js"), /haven-shell-v23/);
  ...
  assert.match(readRepoFile("lib/memory-db.ts"), /MEMORY_DB_VERSION = 5/);
  assert.match(readRepoFile("lib/video-deferred-persist.ts"), /scheduleDeferredVideoPersist/);
  assert.match(readRepoFile("lib/post-seal-memory-guard.ts"), /POST_SEAL_VIDEO_QUIET_MS = 90_000/);
  assert.match(readRepoFile("lib/memory-db.ts"), /STORE_VIDEO_CHUNKS/);
  assert.match(readRepoFile("lib/video-chunk-store.ts"), /putVideoBlobChunked/);
  assert.match(readRepoFile("lib/seal-local-limits.ts"), /MAX_VIDEO_ATTACHMENT_BYTES = 80/);
  assert.match(readRepoFile("src/features/memories/localMemoryStore.ts"), /persistVideoInputs/);
...
check("seal staging phase 3: storage split, cron purge, strategy + limits", () => {
  ...
  assert.match(config, /SEAL_LOCAL_MAX_BYTES = 500 \* 1024 \* 1024/);
  const attachmentLimits = readRepoFile("lib/composer-attachment-limits.ts");
  assert.match(attachmentLimits, /MAX_VIDEO_ATTACHMENT_BYTES/);
  assert.match(readRepoFile("lib/seal-local-limits.ts"), /MAX_VIDEO_ATTACHMENT_BYTES = 80/);
  assert.match(readRepoFile("src/views/NewMemoryPage.js"), /maxAttachmentBytesForMime/);
  assert.doesNotMatch(readRepoFile("src/views/NewMemoryPage.js"), /MAX_ATTACHMENT_SIZE_MB = 50/);
...
check("seal session boundary: no background auto-arm; live size meter on compose", () => {
  ...
  assert.match(appRouter, /shouldMountSealSessionListeners/);
  assert.match(appRouter, /ComposerDraftProvider/);
  assert.match(appRouter, /needsMemoriesProvider/);
  assert.doesNotMatch(readRepoFile("src/features/seal/sealSessionBoundary.ts"), /from "\.\/sealFlowClient"/);
  assert.match(readRepoFile("src/features/seal/sealSessionBoundary.ts"), /sealPrepState/);
  assert.match(readRepoFile("src/features/seal/sealCore.ts"), /shouldMountSealSessionListeners/);
  assert.match(readRepoFile("src/features/seal/sealPrepState.ts"), /clearSealPrepStateLocal/);
  assert.match(newMemory, /sealComposer/);
  assert.match(newMemory, /import\("\.\.\/features\/seal\/sealFinalize"\)/);
  assert.doesNotMatch(newMemory, /from "\.\.\/features\/seal";\n/);
...
check("seal commit persists memories to local timeline", () => {
  ...
  assert.match(readRepoFile("lib/background-sync-client.ts"), /runSealFinalizeNetwork/);
  assert.match(readRepoFile("lib/workers/backgroundSync.worker.ts"), /seal_finalize/);
  assert.match(readRepoFile("lib/post-seal-memory-guard.ts"), /isDeferredDuringPostSealQuiet/);
  assert.match(readRepoFile("lib/post-seal-memory-guard.ts"), /POST_SEAL_VIDEO_QUIET_MS/);
  assert.match(readRepoFile("src/services/offlineSyncQueue.ts"), /isDeferredDuringPostSealQuiet/);
  assert.doesNotMatch(readRepoFile("src/services/lightSyncService.js"), /import \{ flushOfflineSyncQueue \}/);
  assert.match(readRepoFile("src/services/lightSyncService.js"), /import\("\.\/offlineSyncQueue"\)/);
  assert.match(readRepoFile("src/features/seal/sealFlowClient.ts"), /runSealFinalizeNetwork/);
  assert.doesNotMatch(readRepoFile("src/features/seal/sealFlowClient.ts"), /mode: "precheck"/);
  ...
  assert.match(readRepoFile("lib/memory-photo-types.ts"), /isComposerPhotoBlobRow/);
  assert.match(sealFlow, /sealRelayPayloadWithPortableMedia/);
  assert.match(sealFlow, /prepareComposerPhotosForSave/);
  assert.match(readRepoFile("lib/timeline-thumb-cache.ts"), /thumbCacheKey/);
  assert.match(readRepoFile("lib/photo-blob-store.ts"), /getPhotoBlobForMemory/);
  assert.match(readRepoFile("src/hooks/useMemoryPhotoDisplayUrl.js"), /memoryId/);
  assert.match(readRepoFile("lib/composer-photo-utils.ts"), /hydrateComposerPhotosFromMemory/);
  assert.match(readRepoFile("lib/photo-blob-store.ts"), /deletePhotoBlobsForMemoryPhotoIds/);
  assert.match(readRepoFile("lib/memory-photo-types.ts"), /scopedPhotoBlobStoreId/);
  assert.match(readRepoFile("src/services/pairSharingService.js"), /user_owned/);
  assert.match(readRepoFile("src/services/pairSharingService.js"), /remintImportedPhotoRows/);
